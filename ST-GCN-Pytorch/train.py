...
def train_model(model, losser, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_vid, labels in pbar_train:
            ...
            train_corrects += (
                (preds == labels.data.argmax(1)).detach().cpu().numpy().mean()
            )
            ...
            pbar_train.set_postfix(
                OrderedDict(
                    {
                        "Loss": loss.item(),
                        "Memory": "%0.2f GB / %0.2f GB"
                        % (used_memory_before / 1024, total_memory / 1024),
                    }
                )
            )
        ...
        logging.warning(
            "Train: Accuracy: {}, Loss: {}, Time: {}".format(
                epoch_acc,
                epoch_loss,
        ...
        with torch.no_grad():
            ...
            logging.warning(
                "Validation: Accuracy: {}, Loss: {}, Time: {}".format(
                    epoch_acc,
                    epoch_loss,
                    str(datetime.timedelta(seconds=time.time() - last_time)),
                )
