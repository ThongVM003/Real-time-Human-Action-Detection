...
class UCF_JHMDB_Evaluator(object):
    ...
    def evaluate_frame_map(self, model, epoch=1, show_pr_curve=False):
        ...
        # inference
        for iter_i, (batch_frame_id, batch_video_clip, batch_target) in enumerate(
            self.testloader
        ):
            # to device
                ...
                for bi in range(len(batch_scores)):
                    ...
                    if self.dataset == "ucf24":
                        ...
                        if not os.path.exists(
                            "results/ucf_detections/" + self.model_name
                        ):
                        ...
                        if not os.path.exists(
                            "results/jhmdb_detections/" + self.model_name
                        ):
    ...
    def evaluate_video_map(self, model):
        ...
        for i, line in enumerate(lines):
            ...
            for iter_i, (batch_img_name, batch_video_clip, batch_target) in enumerate(
                self.testloader
            ):
                # to device
                batch_video_clip = batch_video_clip.to(model.device)
        ...
        for iou_th in iou_list:
            per_ap = evaluate_videoAP(
                gt_videos, detected_boxes, self.num_classes, iou_th, True
            )
