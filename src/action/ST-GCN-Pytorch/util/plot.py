...
def plot_cm(CM, normalize=True, save_dir="", names_x=(), names_y=(), show=True):
    ...
    if True:
        ...
        array = CM / (
            (CM.sum(0).reshape(1, -1) + 1e-6) if normalize else 1
        )  # normalize columns
        if not normalize:
        ...
        labels_x = (0 < len(names_x) < 99) and len(
            names_x
        ) == 7  # apply names to ticklabels
        labels_y = (0 < len(names_y) < 99) and len(
            names_y
        ) == 7  # apply names to ticklabels
        with warnings.catch_warnings():
            ...
            sn.heatmap(
                ...
                yticklabels=names_y if labels_y else "auto",
            ).set_facecolor((1, 1, 1))
        ...
        plt.title(
            "Accuracy: {}%".format(round(accu * 100)), fontweight="bold", fontsize=20
        )
        if show:
            plt.show()
